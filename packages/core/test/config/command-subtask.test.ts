      ...
      yield* ConfigCommandPlugin.Plugin.effect(
        host({
          ...
          session: {
            ...
            switchAgent: (input) => runtime.session.switchAgent(input),
            switchModel: (input) => runtime.session.switchModel(input),
            prompt: (input) => runtime.session.prompt(input),
          },
        }),
      )
...
const it = testEffect(
  AppNodeBuilder.build(
    ...
    [
      ...
      [PluginSupervisor.node, commandPluginSupervisor],
    ],
  ),
)
...
describe("command subtask execution", () => {
  ...
  it.live("forces a child on the caller's agent when subtask is true without an agent", () =>
    withProject((location) =>
      Effect.gen(function* () {
        const sessions = yield* Session.Service
        const parent = yield* sessions.create({ location, agent: Agent.ID.make("build"), model: parentModel })
