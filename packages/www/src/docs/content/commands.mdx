...
| `subtask`     | No        | Force the command into a child session, or `false` to keep it in place. |
...
When `agent` names an agent whose mode is `subagent`, the command instead runs
in a child session in the background, and the calling session is notified with
the subagent's response when it finishes. The caller keeps its own active agent
and model; only the child adopts the command's agent and model.

Set `subtask: false` to keep such a command in the calling session, where it
behaves like any other command and switches the session's active agent. Set
`subtask: true` to force a child session even when `agent` names a primary
agent, or when the command names no agent at all. A forced command with no
agent runs the child as the calling session's current agent and keeps the
calling session's active model.
